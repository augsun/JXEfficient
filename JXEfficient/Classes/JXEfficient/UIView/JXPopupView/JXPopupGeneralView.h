...
/**
 ...
 [popView show];
 @endcode
 @warning 如果使用默认的 (titleLabel, contentLabel, <button0Label | button1Label>) 部件, 外部如果自定义设置对应的 (titleViewContentH, contentViewContentH, buttonsViewContentH) 部件高度,  将可能被内部重新布局时计算高度<屏幕旋转等情况>覆盖, 则应该使用各对应的 (heightFor_customTitleView, heightFor_customContentView, heightFor_customButtonsView) 回调进行设置. 该类对父类 JXPopupView 的各布局属性进行默认通用<General(目前包括 popupBgViewContentEdgeT, popupBgViewContentEdgeB, titleViewContentH, contentViewToAboveWidget, contentViewContentH, buttonsViewToAboveWidget, buttonsViewContentH)>化, 并提供 3 个部件的 custom 半自定义功能. 如果需要高度自定义, 继承 JXPopupView 抽象类在子类中自定义即可.
 */
...
@property (nonatomic, strong, nullable) UIView *customTitleView; ///< 自定义标题 <若自定义, 则必须一并实现 heightFor_customTitleView, 否则不展示.>, 如果设置, 则 titleLabel 被忽略.
@property (nonatomic, copy, nullable) CGFloat (^heightFor_customTitleView)(void); ///< 自定义标题 高度. 若实现, 标题高度内部不做计算. <除自定义 customTitleView, 否则不建议实现, 只需设置好 titleLabel 相关属性, 内部计算.>

// 内容
@property (nonatomic, readonly) UILabel *contentLabel; ///< 内容 默认 font(14.0), textColor (gray 51), textAlignment(center), numberOfLines(0)
@property (nonatomic, strong, nullable) UIView *customContentView; ///< 自定义内容 <若自定义, 则必须一并实现 heightFor_customContentView, 否则不展示.>, 如果设置, 则 contentLabel 被忽略.
@property (nonatomic, copy, nullable) CGFloat (^heightFor_customContentView)(void); ///< 自定义内容 高度. 若实现, 内容高度内部不做计算. <除自定义 customContentView, 否则不建议实现, 只需设置好contentLabel 相关属性, 内部计算.>
...
@property (nonatomic, strong, nullable) UIView *customButtonsView; ///< 自定义按钮 <若自定义, 则必须一并实现 heightFor_customButtonsView, 否则不展示.>, 如果设置, 则 button0Label button1Label 被忽略.
@property (nonatomic, copy, nullable) CGFloat (^heightFor_customButtonsView)(void); ///< 自定义按钮 高度. 若实现, 按钮高度内部不做计算. <除自定义 customButtonsView, 否则不建议实现, 内部默认 44.0>
